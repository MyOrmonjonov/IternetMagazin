...
<div class="container">
  ...
  <div class="product-card">
    ...
    <div class="product-actions">
      <a href="/update?productId=<%= product.getId() %>&action=decrease">-</a>
      <span><%= product.getCount() %></span>
      <a href="/update?productId=<%= product.getId() %>&action=increase">+</a>
      <a href="/remove?productId=<%= product.getId() %>" class="remove-button">Remove</a>
    </div>
  </div>
  <%
    }
  ...
  <%
    ...
    if (!orderItems.isEmpty()) {
      ...
      int orderId = random.nextInt(51) + 50;
      Order order = new Order(orderId, LocalDateTime.now(), total);
      for (OrderItem item : orderItems) {
        order.addItem(item);
      }
      if (!DB.orders1.contains(order)) {
